...
function addParkingSpace(parkingSpaceData) {
    ...
    fetch(`https://16.170.227.32/parking_space/add/${parkingLotId}`, {
        method: 'POST',
        headers: {
...
function mostrarParkingSpaces(parkingLotId) {
    document.getElementById('parkingLotIdInput').value = parkingLotId;
    fetch(`https://16.170.227.32/parking_space/by-parking-lot/${parkingLotId}`)
    .then(response => response.json())
    .then(parkingSpaces => {
...
function eliminarParkingSpace(spaceId) {
    fetch(`https://16.170.227.32/parking_space/delete/${spaceId}`, {
        method: 'DELETE'
    })
    .then(response => {
        if (response.ok) {
...
function filtrarPorPiso(piso) {
    ...
    fetch(`https://16.170.227.32/parking_space/by-parking-lot/${parkingLotId}/floor/${piso}`)
    .then(response => response.json())
    .then(parkingSpaces => {
