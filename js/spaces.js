document.addEventListener('DOMContentLoaded', function() {
    ...
    fetch('http://localhost:8080/parking_lot')
...
function addParkingSpace(parkingSpaceData) {
    ...
    fetch(`http://localhost:8080/parking_space/add/${parkingLotId}`, {
        method: 'POST',
        headers: {
...
function mostrarParkingSpaces(parkingLotId) {
    document.getElementById('parkingLotIdInput').value = parkingLotId;
    fetch(`http://localhost:8080/parking_space/by-parking-lot/${parkingLotId}`)
    .then(response => response.json())
    .then(parkingSpaces => {
...
function eliminarParkingSpace(spaceId) {
    fetch(`http://localhost:8080/parking_space/delete/${spaceId}`, {
        method: 'DELETE'
    })
    .then(response => {
        if (response.ok) {
...
function filtrarPorPiso(piso) {
    ...
    fetch(`http://localhost:8080/parking_space/by-parking-lot/${parkingLotId}/floor/${piso}`)
    .then(response => response.json())
    .then(parkingSpaces => {
